#ifndef _ZQ_CAMERA_POSE_ESTIMATION_H_
...
namespace ZQ
{
	...
	*/
	template<class T>
	bool ZQ_CameraPoseEstimation::PositCoplanar(int nPts, const T* X3, const T* X2, const T fc[2], const T cc[2], const T kc[5], const T alpha_c, int max_iter, double tol_E, T* rT, T* reproj_err_square, bool zAxis_in)
	{
		if (fc[0] <= 0 || fc[1] <= 0)
			return false;
		T focal_len = fc[0];
		...
		for (int pp = 0; pp < nPts; pp++)
		{
			...
			T tmp_out[2];
			ZQ_CameraProjection::undistort_points_oulu(1, tmp_in, kc, tmp_out);
			...
			for (int cccc = 0; cccc < last_candidates.size(); cccc++)
			{
				...
				*/
				...
				for (int dd = 0; dd < 2; dd++)
				{
					...
					ZQ_CameraProjection::project_points_fun(nPts, X3, cur_rT, fc, cc, kc, alpha_c, cur_X2, zAxis_in);
					cur_E[dd] = 0;
					for (int i = 0; i < nPts; i++)
					{
		...
		if (reproj_err_square != 0)
		{
			...
			if (!zAxis_in)
				cur_rT[5] = -cur_rT[5];
			ZQ_CameraProjection::project_points_fun(nPts, X3, cur_rT, fc, cc, kc, alpha_c, reproj_X2, zAxis_in);
			for (int i = 0; i < nPts; i++)
			{
	...
	*/
	template<class T>
	bool ZQ_CameraPoseEstimation::PositCoplanarRobust(int nPts, const T* X3, const T* X2, const T fc[2], const T cc[2], const T kc[5], const T alpha_c, int max_iter_posit, int max_iter_levmar, double tol_E, T* rT, double& avg_E, bool zAxis_in)
	{
		...
		if (!ZQ_CameraPoseEstimation::PositCoplanar(nPts, X3, X2, fc, cc, kc, alpha_c, max_iter_posit, tol_E, init_rT, reproj_err_square, zAxis_in))
		{
			return false;
		}
	...
	*/
	template<class T>
	bool ZQ_CameraPoseEstimation::_pose_estimation_fun(const T* p, T* hx, int m, int n, const void* data)
	{
		...
		if (!ZQ_CameraProjection::project_points_fun(nPts, X3, p, fc, cc, kc, alpha_c, tmp_X2, zAxis_in))
			return false;
	...
	*/
	template<class T>
	bool ZQ_CameraPoseEstimation::_pose_estimation_jac(const T* p, T* jx, int m, int n, const void* data)
	{
		const Pose_Estimation_Data_Header<T>* ptr = (const Pose_Estimation_Data_Header<T>*)data;
		...
		if (!ZQ_CameraProjection::project_points_jac(nPts, X3, p, fc, cc, kc, alpha_c, jx, (T*)0, (T*)0, (T*)0, (T*)0, zAxis_in))
			return false;
		return true;
	}
	...
	*/
	template<class T>
	bool ZQ_CameraPoseEstimation::pose_estimation_with_init(int nPts, const T* X3, const T* X2, const T fc[2], const T cc[2], const T kc[5], const T alpha_c, int max_iter, T* rT, double& avg_err_square, bool zAxis_in)
	{
