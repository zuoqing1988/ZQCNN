#ifndef _ZQ_MULTI_CAM_CALIBRATION_H_
...
namespace ZQ
{
	...
	template<class T>
	bool ZQ_MultiCamCalibration::_calib_multi_with_known_intrinsic_fun(const T* p, T* hx, int m, int n, const void* data)
	{
		...
		for (int i = 0; i < n_cams; i++)
		{
			...
			for (int j = 0; j < cur_vis_num; j++)
			{
				...
				{
					if (!ZQ_StereoCalibration::_get_left_rT_from_right_rT_fun(checkboard_rT_to_cam0 + cur_checkboard_idx * 6,
						cam0_to_othercam_rT + (i - 1) * 6, checkboard_rT_to_cam_i))
					{
						return false;
					}
				}
				if (!ZQ_CameraCalibration::project_points_fun(N, X3, checkboard_rT_to_cam_i, fc, cc, kc, alpha_c, xp, zAxis_in))
				{
					return false;
				}
	...
	template<class T>
	bool ZQ_MultiCamCalibration::_calib_multi_with_known_intrinsic_jac(const T* p, T* jx, int m, int n, const void* data)
	{
		const Calib_Multi_Data_Header<T>* ptr = (const Calib_Multi_Data_Header<T>*)data;
		int N = ptr->n_pts;
		int n_checkboards = ptr->n_checkboards;
		...
		//for(int i = 0;i < 1;i++)
		{
			...
			for (int j = 0; j < cur_vis_num; j++)
			{
				...
				if (!ZQ_CameraCalibration::project_points_jac(N, X3, right_rT, fc, cc, kc, alpha_c, dxdrT, (T*)0, (T*)0, (T*)0, (T*)0, zAxis_in))
					return false;
		...
		for (int i = 1; i < n_cams; i++)
		{
			...
			for (int j = 0; j < cur_vis_num; j++)
			{
				...
				if (!ZQ_StereoCalibration::_get_left_rT_from_right_rT_fun(right_rT, right_to_left_rT, left_rT))
					return false;
				if (!ZQ_StereoCalibration::_get_left_rT_from_right_rT_jac(right_rT, right_to_left_rT, d_l_rT_d_r_rT, d_l_rT_d_r2l_rT))
					return false;

				if (!ZQ_CameraCalibration::project_points_jac(N, X3, left_rT, fc, cc, kc, alpha_c, dxdrT, (T*)0, (T*)0, (T*)0, (T*)0, zAxis_in))
					return false;
	...
	template<class T>
	bool ZQ_MultiCamCalibration::_calib_multi_with_known_intrinsic_jac_sparse(const T* p, taucs_ccs_matrix*& jx, int m, int n, const void* data)
	{
		const Calib_Multi_Data_Header<T>* ptr = (const Calib_Multi_Data_Header<T>*)data;
		int N = ptr->n_pts;
		int n_checkboards = ptr->n_checkboards;
	...
	*/
	template<class T>
	bool ZQ_MultiCamCalibration::_calib_multi_with_known_intrinsic_with_init(int nCheckboards, int nPts, int nCams, const T* X3, const T* X2, 
	...
	*/
	template<class T>
	bool ZQ_MultiCamCalibration::_calib_multi_with_known_intrinsic_init(int nCheckboards, int nPts, int nCams, const T* X3, const T* X2, const int* visible_num, const int* visible_offset, const int* visible_idx,
		const T* fc_cc_alpha_kc, bool zAxis_in, int max_iter_levmar, T* board_in_cam0_rT, T* cam0_to_othercam_rT, double& avg_err_square)
	{
		...
		for (int i = 0; i < nCams; i++)
		{
			...
			for (int j = 0; j < nCheckboards; j++)
			{
				if (visible_map[i*nCheckboards + j])
				{
					...
					if (!ZQ_CameraCalibration::PositCoplanarRobust(nPts, X3, X2 + cur_offset*nPts * 2, fc,cc,kc,alpha_c, 20 , max_iter_levmar, 1.0,
	...
	template<class T>
	bool ZQ_MultiCamCalibration::_compute_err_calib(int nCheckboards, int nPts, int nCams, const T* X3, const T* X2, const int* visible_num, const int* visible_offset, const int* visible_idx,
		const T* board_in_cam0_rT, const T* cam0_to_othercam_rT, const T* fc_cc_alpha_kc, double& err_std, double& max_err, bool zAxis_in)
	{
