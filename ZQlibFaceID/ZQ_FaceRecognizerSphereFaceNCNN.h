	...
	public:
		...
		virtual bool ExtractFeature(const unsigned char* img, int widthStep, ZQ_PixelFormat pixFmt, float* feat, bool normalize)
		{
			...
			case ZQ_PIXEL_FMT_GRAY:
				...
				input = ncnn::Mat::from_pixels_resize(img, ncnn::Mat::PIXEL_GRAY2RGB, crop_width, crop_height, crop_width, crop_height);
			...
			case ZQ_PIXEL_FMT_BGR:
				...
				input = ncnn::Mat::from_pixels_resize(img, ncnn::Mat::PIXEL_BGR2RGB, crop_width, crop_height, crop_width, crop_height);
				input.substract_mean_normalize(mean_vals, norm_vals);
				break;
			case ZQ_PIXEL_FMT_RGB:
				...
				input = ncnn::Mat::from_pixels_resize(img, ncnn::Mat::PIXEL_RGB, crop_width, crop_height, crop_width, crop_height);
				input.substract_mean_normalize(mean_vals, norm_vals);
				break;
			default:
				return false;
