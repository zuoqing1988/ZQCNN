...
bool EvaluationSphereFaceMiniCaffeOnLFW(const std::string& prototxt_file, const std::string& caffemodel_file, const std::string& out_blob_name,
	const std::string& list_file, const std::string& folder, int max_thread_num = 4, bool use_flip = false)
{
	...
	{
		recognizers[i] = new ZQ_FaceRecognizerSphereFaceMiniCaffe();
...
int main(int argc, const char** argv)
{
	...
	if (!EvaluationSphereFaceMiniCaffeOnLFW(string(argv[1]), string(argv[2]), string(argv[3]), string(argv[4]), string(argv[5]), max_thread_num, use_flip))
		return EXIT_FAILURE;
