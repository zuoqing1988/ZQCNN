...
#include "ZQ_FaceRecognizerArcFaceGrayNCNN.h"
...
int make_database(int argc, char** argv, bool compact, int mode_size)
{
	...
	std::vector<ZQ_FaceRecognizerArcFaceGrayNCNN> recognizer_112X112(max_thread_num);
	std::vector<ZQ_FaceRecognizerSphereFaceGrayNCNN> recognizer_112X96(max_thread_num);
	...
	if (mode_size == mode_size_112X112)
	{
		...
		if (compact)
		{
			if (!ZQ_FaceDatabaseMaker::MakeDatabaseCompactAlreadyCropped(recognizers, database_root, feats_file, names_file, type, show, max_thread_num, true))
			{
				return EXIT_FAILURE;
			}
		}
		...
		{
			if (!ZQ_FaceDatabaseMaker::MakeDatabaseAlreadyCropped(recognizers, database_root, feats_file, names_file, type, show, max_thread_num, true))
			{
				return EXIT_FAILURE;
			}
		}
