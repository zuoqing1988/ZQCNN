...
)
{
	...
	const zq_base_type* in_slice_ptr, *in_row0_ptr, *in_row1_ptr, *in_im_ptr;
	...
	for (w = 0; w < out_W; w++, coord_x += w_step)
	{
		...
		x1[w] = x0[w] + 1;
		sx[w] = coord_x - x0_f;
		x0[w] *= zq_mm_align_size;
		x1[w] *= zq_mm_align_size;
	}
		...
		n++, in_im_ptr += in_sliceStep, out_im_ptr += out_sliceStep)
	{
			...
			c += zq_mm_align_size, in_slice_ptr += in_sliceStep, out_slice_ptr += out_sliceStep)
		{
				...
				h++, coord_y += h_step, out_row_ptr += out_widthStep)
			{
				...
				for (w = 0, out_pix_ptr = out_row_ptr; w < out_W; w++, out_pix_ptr += zq_mm_align_size)
				{
