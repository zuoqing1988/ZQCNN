...
namespace ZQ
{
	...
	class ZQ_CNN_CascadeOnet_Interface
	{
	public:
		using string = std::string;
		ZQ_CNN_CascadeOnet_Interface()
		{
	...
	private:
		ZQ_CNN_Tensor4D_Interface input, onet1_image, onet2_image, onet3_image;
		ZQ_CNN_Net_Interface onet1, onet2, onet3;
	...
	public:
		...
		bool Find(const ZQ_CNN_Tensor4D_Interface& input,
			int xmin, int ymin, int xmax, int ymax,
			std::vector<ZQ_CNN_BBox>& results, int nIters = 3)
		{
			results.clear();
			...
			for (int i = 3; i < nIters; i++)
				nets.push_back(&onet3);
			std::vector<ZQ_CNN_Tensor4D_Interface*> onet_images;
			...
			for (int i = 0; i < nIters; i++)
			{
				...
				const ZQ_CNN_Tensor4D_Interface_Base* prob = nets[i]->GetBlobByName("prob1");
				const ZQ_CNN_Tensor4D_Interface_Base* location = nets[i]->GetBlobByName("conv6-2");
				if (prob == 0)
				{
