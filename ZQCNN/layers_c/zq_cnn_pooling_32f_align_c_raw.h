...
void zq_cnn_avgpooling_nopadding_suredivided_kernel2x2_omp(
	const float* in_tensor4D_data,
	int in_N,
	int in_H,
	int in_W,
...
void zq_cnn_avgpooling_nopadding_suredivided_kernel3x3_omp(
	const float* in_tensor4D_data,
	int in_N,
	int in_H,
	int in_W,
...
void zq_cnn_avgpooling_nopadding_suredivided_kernel5x5_omp(
	const float* in_tensor4D_data,
	int in_N,
	int in_H,
	int in_W,
...
void zq_cnn_avgpooling_nopadding_suredivided_general_omp(
	const float* in_tensor4D_data,
	int in_N,
	int in_H,
	int in_W,
...
void zq_cnn_avgpooling_nopadding_nodivided_general_omp(
	const float* in_tensor4D_data,
	int in_N,
	int in_H,
	int in_W,
...
#pragma omp parallel for schedule(static,chunk_size) num_threads(thread_count)
	...
	{
		zq_mm_type val;
		zq_mm_type scale = zq_mm_set1_ps(1.0 / (kernel_H*kernel_W));
		int n, out_h, out_w, kh, kw;
		const float* in_pix_ptr, *in_row_ptr, *in_slice_ptr;
		float* out_pix_ptr, *out_row_ptr, *out_slice_ptr;
		const float* cur_pix_ptr, *cur_row_ptr;
