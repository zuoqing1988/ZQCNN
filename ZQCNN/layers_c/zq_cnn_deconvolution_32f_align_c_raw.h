...
)
{
					...
					out_w++, out_pix_ptr += out_pixelStep)
				{
						...
						out_c++, cur_filter_slice_ptr += filter_sliceStep, out_c_ptr++)
					{
						...
						for (kc = 0, cur_in_c_ptr = cur_in_pix_ptr, cur_filter_c_ptr = cur_filter_pix_ptr;
							kc < filter_C;
							kc += zq_mm_align_size8, cur_in_c_ptr += zq_mm_align_size8, cur_filter_c_ptr += zq_mm_align_size8)
						{
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr), zq_mm_load_ps(cur_filter_c_ptr), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size2), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size2), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size3), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size3), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size4), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size4), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size5), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size5), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size6), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size6), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size7), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size7), sum);
						}
						zq_mm_store_ps(q, sum);
						*out_c_ptr = zq_final_sum_q;
					}
	...
	else if (out_N %zq_mm_align_size4 == 0)
	{
		for (out_n = 0, in_slice_ptr = in_tensor4D_data, out_slice_ptr = out_tensor4D_data;
			out_n < out_N;
			out_n++, in_slice_ptr += in_sliceStep, out_slice_ptr += out_sliceStep)
				...
				out_h++, out_row_ptr += out_widthStep)
			{
					...
					out_w++, out_pix_ptr += out_pixelStep)
				{
						...
						out_c++, cur_filter_slice_ptr += filter_sliceStep, out_c_ptr++)
					{
						...
						for (kc = 0, cur_in_c_ptr = cur_in_pix_ptr, cur_filter_c_ptr = cur_filter_pix_ptr;
							kc < filter_C;
							kc += zq_mm_align_size4, cur_in_c_ptr += zq_mm_align_size4, cur_filter_c_ptr += zq_mm_align_size4)
						{
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr), zq_mm_load_ps(cur_filter_c_ptr), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size), sum);
							sum = zq_mm_fmadd_ps(zq_mm_load_ps(cur_in_c_ptr + zq_mm_align_size2), zq_mm_load_ps(cur_filter_c_ptr + zq_mm_align_size2), sum);
