...
int main()
{
	...
	//cv::resize(image0, image0, cv::Size(), 3, 3);
	...
	ZQ_CNN_MTCNN_Interface<ZQ_CNN_Net, ZQ_CNN_Tensor4D_NHW_C_Align128bit, ZQ_CNN_Tensor4D> mtcnn;
	...
	if (use_pnet20)
	{
		if (landmark106)
		{
			...
			if (!mtcnn.Init(
				...
				thread_num, true,
				"model/det5-dw112.zqparams", "model/det5-dw112-310.nchwbin"
		...
		mtcnn.SetPara(image0.cols, image0.rows, 22, 0.5, 0.6, 0.8, 0.4, 0.5, 0.5, 0.709, 3, 20, 4, special_handle_very_big_face, true);
	}
	else
	{
#if defined(_WIN32)
