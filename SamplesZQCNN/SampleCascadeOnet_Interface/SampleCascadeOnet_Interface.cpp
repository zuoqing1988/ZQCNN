...
int main()
{
	...
	ZQ_CNN_CascadeOnet_Interface<ZQ_CNN_Net, ZQ_CNN_Tensor4D_NHW_C_Align128bit, ZQ_CNN_Tensor4D> net1;
	...
	if (!net1.Find(ori_img.data, ori_img.cols, ori_img.rows, ori_img.step[0], 0, 0, ori_img.cols, ori_img.rows, results, 10))
	{
		std::cout << "failed\n";
		return EXIT_FAILURE;
	}
	...
	for (int i = 0; i < results.size(); i++)
	{
		...
		rectangle(draw_img, rect, cv::Scalar(0, results[i].score * 255, 0), 1);
	}
